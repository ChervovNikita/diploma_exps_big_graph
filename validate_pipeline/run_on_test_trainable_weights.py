...
raw_model = TAGConvModelTrainableWeights(num_features=num_features, num_classes=len(labels)).to(device)
raw_model.load_state_dict(torch.load(f'checkpoints/{EXP_NAME}_best.pt'))
model = SingleDeviceWrapper(raw_model, device)
