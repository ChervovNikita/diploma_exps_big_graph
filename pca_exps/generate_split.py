...
data = pd.read_csv('../CR_real_masks_more_labeled_veritices_agreed_with_pca_features.csv')
data['node_id1'] -= 1
data['node_id2'] -= 1
...
pca_data = pd.read_csv('3382_sampeles_in_pca_plot_over_80_central_russia_pca_data_anonymized.eigenvec', sep=r'\s+')
pca_data['IID'] -= 1
pca_features = np.zeros((max_node + 1, 20), dtype=np.float32)
for _, row in pca_data.iterrows():
    uid = int(row['IID'])
    pca_row = np.array([row[f'PC{i}'] for i in range(1, 21)])
    pca_features[uid] = pca_row
