...
class SingleDeviceWrapper(torch.nn.Module):
    ...
    def forward(self, data_list, tabm_seed=0):
        ...
        return self.module(batch, tabm_seed), batch
...
base_model = TAGConvModel(num_features=num_features, num_classes=len(labels), tabm_inits=2).to(
    "cuda:1"
)

model = SingleDeviceWrapper(base_model, "cuda:1")
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='Eval', leave=False):
            with torch.amp.autocast('cuda', enabled=use_amp):
                logits = None
                for tabm_seed in range(base_model.tabm_inits):
                    if isinstance(model, SingleDeviceWrapper):
                        logits_now, batch_data = model(batch, tabm_seed)
                        if logits is None:
                            logits = logits_now
                        else:
                            logits += logits_now
                        ...
                        logits_now = model.module(batch_data, tabm_seed)
                        if logits is None:
                            logits = logits_now
                        else:
                            logits += logits_now
...
class_weights = torch.tensor([max(class_counts) / c for c in class_counts], dtype=torch.float).to(torch.device("cuda:1"))
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, batch in enumerate(loop):
        ...
        for tabm_seed in range(base_model.tabm_inits):
            with torch.amp.autocast('cuda', enabled=use_amp):
                if isinstance(model, SingleDeviceWrapper):
                    logits, batch_data = model(batch, tabm_seed)
                else:
                    batch_data = Batch.from_data_list(batch).to(device)
                    logits = model.module(batch_data, tabm_seed)
