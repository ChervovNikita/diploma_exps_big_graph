...
# ============== Experiment Name ==============
EXP_NAME = 'exp_mask_soft_ce_0.25'
...
class SmoothCrossEntropyLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        if self.weight is not None:
            class_weight = self.weight[target].unsqueeze(1)
            nll_loss = nll_loss * class_weight
        smooth_loss = -log_prob.mean(dim=-1, keepdim=True)
        loss = (1 - self.alpha) * nll_loss + self.alpha * smooth_loss
        return loss.mean()

# criterion = torch.nn.CrossEntropyLoss(weight=class_weights)
criterion = SmoothCrossEntropyLoss(alpha=0.1, weight=class_weights)
